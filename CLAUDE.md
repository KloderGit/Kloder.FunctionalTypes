...
`FunctionalTypes.Maybe` (`Maybe/Maybe.cs`, `Some.cs`, `None.cs`) is an `Option`-style `Maybe<T>` — a third independent type alongside the `Result` trio and `Either`, following the exact same shape (abstract class + `Some`/`None` sealed subclasses, static `Maybe<T>.Some(value)`/`Maybe<T>.None()` factories). Its operations mirror `Result`'s, minus anything that would need an error payload `None` doesn't have:

- `Map`, `Bind`, `Match(Func<T,TR> some, Func<TR> none)`, `Tap`/`TapNone` (the `None`-side counterpart of `TapError`, but parameterless — `None` carries nothing to pass it), `Check(Predicate<T>)` (fails → becomes `None`, no message), `Deconstruct(out bool hasValue, out T? value)`.
...
- `MaybeReferenceExtensions.cs` — `ToMaybe<T>(this T? value) where T : class`: `null` → `None`, else `Some`.
- `MaybeValueExtensions.cs` — `ToMaybe<T>(this T value) where T : struct` (always `Some`) and `ToMaybe<T>(this T? value) where T : struct` (`Nullable<T>`: `null` → `None`, else `Some`).
- `MaybeSequenceExtensions.cs` — `ToMaybe<T>(this IEnumerable<T>? value)`: `null` *or empty* → `None`, else `Some` of the sequence materialized once (reuses it as-is if it's already an `IReadOnlyCollection<T>`, otherwise `.ToList()`s it — never enumerates a lazy sequence twice).

**Gotcha (real, not hypothetical — caught a test failure while building this):** `MaybeSequenceExtensions.ToMaybe` only wins overload resolution when the argument's *static* type is `IEnumerable<T>` itself. When a variable is statically typed as a concrete reference type instead — `List<T>`, `T[]`, etc. — `MaybeReferenceExtensions.ToMaybe<T> where T : class` wins instead, because C# always prefers an identity-match candidate (`T = List<int>` needs no conversion) over one needing an interface conversion (`IEnumerable<T>` from `List<int>` does), regardless of which is "more specific" generically. That reference overload only null-checks; it has no idea about "empty means `None`". So `new List<int>().ToMaybe()` silently returns `Some(emptyList)`, not `None` — the empty-check only fires when the variable is declared/typed as `IEnumerable<T>` (e.g. it came back from a method whose return type is `IEnumerable<T>`), not when it's a concrete collection type. This was a deliberate, discussed trade-off (see `MaybeSequenceExtensions.ToMaybe`'s XML doc) — not a bug to "fix" by renaming the method; `null → None` still works correctly either way, only the empty-check is affected.
...
**Gotcha when writing tests (or any call site) against `Traverse`/`Sequence`:** a lambda/array mixing `new Success<T>(...)` and `new Failure<T>(...)` branches (e.g. `v == 2 ? new Failure<int>(msg) : new Success<int>(v)`, or `new[] { new Success<int>(1), new Failure<int>("x") }`) fails to compile — `CS0411`/`CS0826`/`CS1662` — because `Success<T>` and `Failure<T>` have no common type on their own; the compiler needs to be told the target is `Result<T>` (cast one branch, or declare the array as `Result<T>[]`) before it'll unify them. `Maybe<T>.Some(...)`/`.None()` don't have this problem because those are *static factory methods already typed to return `Maybe<T>`*, unlike `Result`/`Result<T,TError>`, which have no equivalent factory and are only ever constructed via the concrete `Success`/`Failure` subclasses directly.
