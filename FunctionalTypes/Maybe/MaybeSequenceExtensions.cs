...
public static class MaybeSequenceExtensions
{
    ...
    /// becomes <c>None</c>, otherwise <c>Some</c> of the materialized collection.
    /// </summary>
    /// <remarks>
    ...
    /// always wins. That overload only null-checks — it does not apply the "empty means None" rule.
    ...
    {
        if (value is null)
            return Maybe<IReadOnlyCollection<T>>.None();

        var materialized = value as IReadOnlyCollection<T> ?? value.ToList();
        return materialized.Count == 0
            ? Maybe<IReadOnlyCollection<T>>.None()
            : Maybe<IReadOnlyCollection<T>>.Some(materialized);
    }
}
